<template>
  <Layout>
    <section class="content-inner inner flex flex-col self-center lg:w-1300 md:w-full">
      ...
      <article class="food-wrap">

        <h1 class="section-title">{{cont.pageFood.sectionTitle}}</h1>
      
        <h2 class="sub-section-title">{{cont.pageFood.articlesTitle}}</h2>

             <div class="filter-wrap flex flex-row justify-between">
              <div class="filter-search flex">
                  <input type="text" class="form--text search-form" :placeholder="cont.pageFood.searchPlaceholder" id="search" v-model="search" />
              </div>
            </div>

            <div class="search-results-wrap" v-if="filterArticles.length > 0">

              <div class="current-search" v-if="search"><h4>{{cont.pageFood.searchResults}} <span class="bolded">{{ this.search }}</span></h4></div>
                    ...
                    <g-link :to="node.path">
                      ...
                      <div class="articles-card-content">
                        <div class="articles-card-info inline-flex justify-between">
                          ...
                          <p class="read-more">{{cont.pageFood.readFullArticle}}<i class="fas fa-caret-right"></i></p>
                      </div>
                    </g-link>
                  </figure>
              </div>
            ...
            <div class="no-results-wrap" v-else>
              <h4 class="no-results">{{cont.pageFood.noResults}}</h4>
            </div>
            ...
            <div class="articles-cards-wrap grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-0 gap-x-0 md:gap-x-10" v-if="search.length === 0">
              <figure class="articles-card" v-for=" { node } of $page.foodArticles.edges" :key="node.id" :post="node.title">
                <g-link :to="node.path">
            ...
            <div class="articles-pagination-wrap flex justify-center" v-if="search.length === 0">
              <Pager class="pager-container" linkClass="pager-container-link" :info="$page.foodArticles.pageInfo" />
            </div>

        <h2 class="sub-section-title py-10">{{cont.pageFood.shopTitle}}</h2>

        <div class="feat-products-carousel-wrap hidden md:block">
          <VueSlickCarousel v-bind="settings" >
            <div class="product-slide-card flex flex-col" v-for="product in cont.pageFood.foodProducts" :key="product.id">
              <a :href="product.link">
        ...
        <h2 class="sub-section-title py-10">{{cont.pageFood.nutrisiontsTitle}}</h2>

        <div class="nutrionists-grid grid sm:grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-5">
...
<page-query>
query ($page: Int) {
  foodArticles: allWordpressPost(perPage: 6, sortBy: "date", order:DESC, page: $page, filter: { categories: { id: {in: ["295"]} }})  @paginate {
    pageInfo {
      totalPages
      currentPage
    }
   ...
   computedQuery: allWordpressPost(filter: { categories: { id: {in: ["295"]} }}) {
    edges {
      node {
        id
        title
...
export default {
  ...
  metaInfo() {
    return {
      title: this.cont.pageFood.metaTitle + ' - MammaFitness',
      meta: [
        { name: "description", content: this.cont.pageFood.metaDescription }
      ],
      link: [
        //{ rel: 'canonical', href: this.$page.metadata.siteUrl + this.$page.metadata.pathPrefix + this.cont.pageFood.canoicalLink },
      ]
    }
  },
    data: function() {
  ...
  mounted(){
    ...
    this.after_mount({
        page: { type: 'FoodThemePage', vpTitle: this.cont.pageFood.metaTitle +  ' - MammaFitness', vpUrl: location.pathname }
    });
  },
}
</script>
